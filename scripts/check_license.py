...
class UnexpectedBinaryFileError(RuntimeError):
    ...
    def __init__(self, file_path: str | Path):
        message = f"The file could not be read because it is binary: {
            str(file_path)}"
...
def get_target_files(
    ...
    # get all files:
    all_files_path = Path(abs_target_dir).rglob("*")
    all_files = [file_.absolute() for file_ in all_files_path if file_.is_file()]
...
def check_copyright_notice(
    ...
    if global_copyright.text is not None:
        copyright_cleaned = "\n".join(copyright_lines[: global_copyright.n_lines])
    ...
    copyright_cleaned = "\n".join(copyright_lines[: len(template_line)])
